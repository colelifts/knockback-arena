...
describe('authoritative match movement and combat', () => {
  ...
  it('turns simultaneous punches into a clash instead of two full hits', () => {
    const simulation = playingSimulation();
    simulation.testSetPlayerPosition('attacker', { x: -4, y: 1.75, z: 20 });
    simulation.testSetPlayerPosition('target', { x: 4, y: 1.75, z: 20 });
    simulation.setInput('attacker', input(1, { facingX: 1, punch: true }));
    simulation.setInput('target', input(1, { facingX: -1, punch: true }));
    simulation.step();
    simulation.setInput('attacker', input(2, { facingX: 1 }));
    simulation.setInput('target', input(2, { facingX: -1 }));
    for (let tick = 0; tick < 5; tick += 1) simulation.step();
    const [attacker, target] = simulation.snapshot().players;
    expect(attacker?.action).toBe('hit');
    expect(target?.action).toBe('hit');
    expect(attacker?.velocity.x).toBeCloseTo(-5.5, 1);
    expect(target?.velocity.x).toBeCloseTo(5.5, 1);
  });
